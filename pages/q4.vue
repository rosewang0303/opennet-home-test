...
function cache<T>(fn: () => T, time = 5000) {
  let cachedValue: T | null = null;
  let lastTime = 0;

  return function (): T {
    const now = Date.now();
    if (cachedValue !== null && now - lastTime < time) {
      // Return cached value if within cache duration
      return cachedValue;
    }
    // Otherwise, call the original function and update cache
    cachedValue = fn();
    lastTime = now;
    return cachedValue;
  };
...
function run() {
  const intervalId = setInterval(() => {
    count++;
    const number = cachedGetData();
    data.value = { number, count };
  }, 1000);
}
</script>
